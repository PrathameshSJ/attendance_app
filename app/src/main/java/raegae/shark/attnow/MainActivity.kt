            ...
            while (isActive) {
                ...
                try {
                    ...
                    if (file.exists() && file.length() > 0) {
                        android.util.Log.d("Watcher", "Found pending import file!")
                        // Wait a bit to ensure write is done
                        delay(1000)

                        AppGlobalState.setImporting(true)
                        val db = AppDatabase.getDatabase(applicationContext)
                        val manager = AttendanceExcelManager(applicationContext, db)
                        ...
                        android.util.Log.d("Watcher", "Import processed. Result posted.")
                    }
                } catch (e: Exception) {
...
fun FabForRoute(currentRoute: String?, navController: NavController) {
    when (currentRoute) {
        AppDestinations.HOME.route -> {
            ...
            FloatingActionButton(onClick = { navController.navigate("home_add_existing") }) {
